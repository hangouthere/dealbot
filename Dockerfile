...
FROM base AS production
      ...
      # RTFM :)
      ENV CRONTAB_PATTERN_IMPORT                      "*/5 * * * *"
      ENV CRONTAB_PATTERN_NOTIFY                      "*/10 * * * *"
      ENV CRONTAB_PATTERN_PRUNE_ERRORED_NOTIFICATIONS "0 0 0 * * MON *"
      ENV CRONTAB_PATTERN_PRUNE_OLD_DATA              "0 0 0 1 1/1 * *"
      ENV CRONTAB_PATTERN_PRUNE_REPORTS               "0 0 0 1 1 * *"
      ENV CRONTAB_PATTERN_REPORT_COMPARE              "2 0 0 * * MON *"
      ENV CRONTAB_PATTERN_REPORT_PERIODIC             "1 0 0 * * MON *"
      ...
      COPY --from=production-build --chown=node:node ${BUILD_PATH}/knexfile.js .
      COPY --from=production-build --chown=node:node ${BUILD_PATH}/knex ./knex
